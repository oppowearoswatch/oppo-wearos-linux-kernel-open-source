	...
	size_t count, loff_t *ppos)
{
    ...
    if (ret == tfa_error_ok) {
        ...
        if (cal_profile < 0) {
           ...
           tfa98xx->i2c->addr);
/* WSW.Audio.Kernel, 2019-09-21 add for tfa9895 no calibrate profile*/
            cal_profile = 0;
       }
...
static int tfa98xx_ext_reset(struct tfa98xx *tfa98xx)
{
	if (tfa98xx && gpio_is_valid(tfa98xx->reset_gpio)) {
		gpio_set_value_cansleep(tfa98xx->reset_gpio, !tfa98xx->reset_active_low);
		mdelay(1);
		gpio_set_value_cansleep(tfa98xx->reset_gpio, tfa98xx->reset_active_low);
		mdelay(1);
	}
...
static int tfa98xx_init_gpio(struct device *dev, struct tfa98xx *tfa98xx)
{
	...
	if (!no_reset) {
		...
		tfa98xx->reset_gpio = of_get_named_gpio_flags(np, "reset-gpio", 0, &flags);
		if (gpio_is_valid(tfa98xx->reset_gpio)) {
			tfa98xx->reset_active_low = !!(flags & OF_GPIO_ACTIVE_LOW);
			dev_dbg(dev, "reset-polarity:%d\n", !tfa98xx->reset_active_low);

			ret = devm_gpio_request_one(dev, tfa98xx->reset_gpio,
				tfa98xx->reset_active_low ? GPIOF_OUT_INIT_LOW : GPIOF_OUT_INIT_HIGH,
				"TFA98XX_RST");
			if (ret)
				return ret;
		} else
	...
	if (!no_start) {
		...
		if (gpio_is_valid(tfa98xx->irq_gpio)) {
			ret = devm_gpio_request_one(dev, tfa98xx->irq_gpio,
	...
	const struct i2c_device_id *id)
{
	...
	ret = tfa98xx_init_gpio(&i2c->dev, tfa98xx);
	if (ret) {
